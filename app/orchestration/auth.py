...
async def start_install(adapter: ProviderAdapter) -> str:
    state = generate_state()
    return await adapter.build_authorize_url(state)


async def complete_install(session: AsyncSession, adapter: ProviderAdapter, code: str, state: str) -> str:
...
async def ensure_fresh_access_token(
    session: AsyncSession, adapter: ProviderAdapter, install: Install
) -> Install:
    ...
    try:
        tokens = await adapter.refresh_access_token(install.refresh_token)
    ...
    install = await upsert_install(session, hub_id=install.hub_id, tokens=tokens)
    await session.commit()
    await session.refresh(install)
    return install
